...
#       Modified: 23-Nov-16 to fix encoding issue
#################################################
...
file_list=""
rowdata_files = []
rowdata_files.extend(os.listdir("D:\\FPDS-BOT\\INPUT"))
...
for k, name in enumerate(rowdata_files):
	...
	if file_list!='':
		...
		with open('D:\\FPDS-BOT\\INPUT\\'+file_list, 'rb') as mycsvfile:
			...
			for row in thedata:
				...
				try:
					with open('D:\\FPDS-BOT\\OUTPUT\\'+case_number+'_MICA.txt') as fp:
						...
						for item in rowdata_MICA:
							if item.find("PERS:Customer Name") != -1:
								position = rowdata_MICA.index(item)
								subrowdata_MICA = rowdata_MICA[position:]
								rowdata_MICA = subrowdata_MICA
								#print "{name}".format(iteration=k, name=name)
